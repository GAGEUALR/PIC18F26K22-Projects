...
void initializeLCD() {
    ...
    PORTCbits.RC1 = 0;
    PORTCbits.RC2 = 0;                 //unlock LCD
    PORTA = 0x30;
    enable();
    
    __delay_ms(15);         //wait for more than 15 milliseconds
...
void interrupt keyPressed() {
    ...
    if (INTCONbits.RBIF) {                  //if interrupt flag is ready
        ...
        int i = 0;           //create variable to loop with, initialize as empty
        
        for (i = 0; i < 4; i++) {       //start loop through keypad
            ...
            if (key != '\0')  // breaks loop if key is found, keeping value given
                break;
        }
